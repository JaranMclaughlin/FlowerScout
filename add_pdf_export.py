...
# 2. Refactor _filtered into a reusable helper, so export can filter a
#    freshly-fetched full dataset the same way the on-screen table does
...
#    Matched by regex (not exact text) since the file has mojibake
#    artifacts from earlier encoding issues that make exact matching fragile.
pattern = re.compile(
