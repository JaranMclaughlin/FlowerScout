...
class _AnalyticsScreenState extends ConsumerState<AnalyticsScreen> {
  ...
  Widget build(BuildContext context) {
    ref.watch(stringsProvider); // keep reactive - use getter s
  ...
  Widget _buildChartCard(ReportStats stats) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _legend(),
        const SizedBox(height: 16),
        SizedBox(height: 200, child: _chartType == s.barLabel ? _barChart(stats) : _lineChart(stats)),
      ]),
    );
  }

  Widget _segmented() => Row(children: [s.barLabel, s.lineLabel].map((t) {
  ...
  Widget _buildSeverityCard(ReportStats stats) {
    ...
    final sevs = [
      ...
      (s.highLabel, pct('High'), _AP.red),
      (s.mediumLabel, pct('Medium'), _AP.amber),
