...
class ReportStats {
  ...
  static ReportStats empty(String lang) => ReportStats(
    ...
    chartLabels: AppStrings.of(lang).chartLabelsWeek,
  );
...
Future<List<InspectionRecord>> fetchInspectionRecords(
  ...
  return (rows as List).map((r) {
    ...
    return InspectionRecord.fromRow(row, lang);
  }).toList();
}
...
final inspectionRecordsProvider =
  ...
  return fetchInspectionRecords(db, filter, lang);
});
...
final reportStatsProvider =
  ...
  final lang = ref.watch(localeProvider);
  return ReportStats.fromInspections(records, filter.period, filter.since, lang);
});
