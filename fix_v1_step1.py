...
# Step 1: Delete dead SQLite files
for f in ['lib/core/database/database_service.dart', 'lib/core/database/scouting_repository.dart']:
    p = pathlib.Path(f)
    if p.exists():
        p.unlink()
        print(f'Deleted {f}')

# Step 2: Fix scouting_screen.dart - wire trail tracking + duplicate submit guard + i18n
scouting = r"""import 'dart:async';
...
pathlib.Path('lib/features/scouting/presentation/scouting_screen.dart').write_text(scouting, encoding='utf-8')
print('scouting_screen.dart done')